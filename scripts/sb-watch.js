...
function _processFile(filePath, watchEvent) {
    ...
    if (filePath.match(/\.pug$/)) return _handlePug(filePath, watchEvent);
    if (filePath.match(/\.scss$/) && watchEvent === 'change') return _handleSCSS();
    if (filePath.match(/src\/js\//)) return renderScripts();
    if (filePath.match(/src\/assets\//)) return renderAssets();
}
...
function _renderAllPug() {
    ...
    _.each(allPugFiles, (value, filePath) => renderPug(filePath));
}
